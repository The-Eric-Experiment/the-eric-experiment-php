...
<?php foreach ($main_menu as $i => $item): ?>
    <?php if (!empty($item['path'])): ?>
        ...
        $path = $item['path'];
        ?>
        <a data-menu-item="true" data-menu-icon="<?= !empty($item['icon']) ? $item['icon'] : ''; ?>"
            data-menu-label="<?= !empty($item['label']) ? $item['label'] : ''; ?>" href="<?= $path; ?>" target="_top" <?php if (!empty($item['image_on'])): ?> onmouseover="changeImages('image<?= $i + 1; ?>','image<?= $i + 1; ?>on')"
                onmouseout="changeImages('image<?= $i + 1; ?>','image<?= $i + 1; ?>off')" <?php endif; ?>>
        <?php endif; ?>
        <img src="/contents/public/menu/<?= $item['image_off'] ?>"
            alt="<?= !empty($item['label']) ? $item['label'] : ''; ?>" border="0" name="image<?= $i + 1; ?>">
        <?php if (!empty($item['path'])): ?>
        </a>
    <?php endif; ?>
